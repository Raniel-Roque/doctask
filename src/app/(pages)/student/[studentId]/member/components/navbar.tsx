  ...
  return (
    ...
    <header className="bg-white shadow-md">
      ...
      <div className="flex items-center justify-between px-6 py-2 bg-gray-200">
        <Link href={`/student/${studentId}/member/home`} className="flex items-center gap-3">
          <Image
            src="/doctask.ico"
            alt="Logo"
            width={45}
      ...
      <nav className="bg-[#B54A4A] px-6 py-3">
        <ul className="flex items-center gap-6 text-white font-medium">
          ...
          <li>
            <Link
              href={`/student/${studentId}/member/docs`}
              className={`flex items-center gap-2 hover:text-gray-300 transition-colors duration-200 ${isActive(`/student/${studentId}/member/docs`) ? 'underline italic' : ''}`}
            >
              <FaFileAlt size={20} />
              Documents
            </Link>
