...
const isOverlapping = (
  ...
  return !(
    selection1.to <= selection2.from || selection2.to <= selection1.from
  );
};
...
export const CollaborativeHighlighting =
  Extension.create<CollaborativeHighlightingOptions>({
              ...
              options.others.forEach((user) => {
                if (user.presence.selection) {
                  ...
                  if (from >= 0 && to >= from && to <= tr.doc.content.size) {
                    ...
                    const hasConflict = options.others.some(
                      (otherUser) =>
                        otherUser.id !== user.id &&
                        otherUser.presence.selection &&
                        isOverlapping(otherUser.presence.selection, {
                          from,
                          to,
                        }),
                    );
